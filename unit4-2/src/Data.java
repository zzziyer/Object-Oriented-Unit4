...
public class Data {
    ...
    public void rule2() throws UmlRule002Exception {
        HashSet<MyClass> temp = new HashSet<>();
        temp.addAll(myClassHashMap.values());
        check.rule002(temp);
    }

    public void rule8() throws UmlRule008Exception {
        HashSet<UmlGeneralization> temp1 = new HashSet<>();
        HashSet<UmlInterfaceRealization> temp2 = new HashSet<>();
        temp1.addAll(umlGeneralization.values());
        temp2.addAll(umlInterfaceRealization.values());
        check.rule008(temp1, temp2,
                umlClass, umlInterface);
    }

    public void rule9() throws UmlRule009Exception {
        HashSet<UmlGeneralization> temp1 = new HashSet<>();
        HashSet<UmlInterfaceRealization> temp2 = new HashSet<>();
        temp1.addAll(umlGeneralization.values());
        temp2.addAll(umlInterfaceRealization.values());
        check.rule009(temp1, temp2,
                umlClass, umlInterface);
    }
    ...
    private void buildAssociation() {
        for (UmlAssociation k :
            ...
            if (umlClass.containsKey(id1) && umlClass.containsKey(id2)) {
                ...
                myClassHashMap.get(id1).putEnd(
                        umlAssociationEnd.get(d2).getName());
                myClassHashMap.get(id2).putEnd(
                        umlAssociationEnd.get(d1).getName());
